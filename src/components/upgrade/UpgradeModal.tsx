...
export function UpgradeModal() {
  ...
  const modal        = upgradeModal; // narrowed — no es null más allá de aquí
  const accentColor  = modal.targetPlan === 'premium' ? '#7C3AED' : BRAND_COLORS.primary;

  function handleCta() {
    closeUpgradeModal();
    navigate(modal.ctaPath ?? '/app/planes');
  }

  function handleSecondary() {
    closeUpgradeModal();
    if (modal.secondaryPath) {
      navigate(modal.secondaryPath);
    }
    // Sin secondaryPath: solo cierra (comportamiento original)
  }
    ...
    >
      ...
      >
        <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.06em', color: accentColor, textTransform: 'uppercase', marginBottom: 8 }}>
          {modal.targetPlan === 'premium' ? 'Shelwi PREMIUM' : 'Shelwi PRO'}
        </div>
        <div style={{ fontSize: 17, fontWeight: 800, color: '#0F172A', marginBottom: 10 }}>{modal.title}</div>
        <p style={{ fontSize: 13.5, color: '#475569', lineHeight: 1.6, marginBottom: modal.bullets?.length ? 14 : 20, whiteSpace: 'pre-line' }}>{modal.message}</p>
